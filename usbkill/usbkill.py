...
def lsusb():
    ...
    if CURRENT_PLATFORM.startswith("DARWIN"):
        ...
        return DeviceCountSet(
            DEVICE_RE[0].findall(subprocess.check_output("lsusb", shell=True).decode('utf-8').strip()))
...
def startup_checks():
    ...
    if settings['melt_usbkill'] or len(settings['folders_to_remove'] + settings['files_to_remove']) > 0:
        ...
        if not settings['remove_file_cmd'].startswith('srm'):
            sys.exit(
                "[ERROR] remove_file_command should start with `srm'. srm should be used for automated data overwrite.\n")
    ...
    if settings['do_wipe_ram']:
        ...
        if not settings['wipe_ram_cmd'].startswith('sdmem'):
            sys.exit(
                "[ERROR] wipe_ram_cmd should start with `sdmem'. sdmem should be used for automated data overwrite.\n")
    ...
    if settings['do_wipe_swap']:
        ...
        if not settings['wipe_swap_cmd'].startswith('sswap'):
            sys.exit(
                "[ERROR] wipe_swap_cmd should start with `sswap'. sswap should be used for automated data overwrite.\n")
